...
router.post('/Login', function(req,res){
    ...
    var date=req.body.lastDate;
    db.Select(connection,"Select User_name,User_password,lastDate from Users_tbl where User_name='"+name+"' and User_password='"+password+"'",function (ans) {
        if(ans.length==1)
        {
            var t="UPDATE Users_tbl SET lastDate = '"+date+"' where User_name='"+name+"'";
            db.Insert(t, config);
...
router.post('/Register', function(req,res){
    ...
    db.Select(connection,"Select User_name from Users_tbl where User_name='"+name+"'",function (ans) {
        ...
        {
            db.Insert("INSERT INTO Users_tbl" + "(User_name, User_firstname, User_lastname, User_email, User_adress, User_password, User_country, User_reqans1, User_reqans2, lastDate)" + "VALUES ('" + name + "', '"+ firstname + "', '"+ lastname + "', '"+ email + "', '"+ adress + "', '"+ password + "', '"+ country + "', '"+ reqans1 + "', '"+ reqans2 + "',"+date+"');",config);
...
router.post('/updatelastLogin', function(req,res){
    var name = req.body.User_name;
    var date=req.body.lastDate;
    db.Select(connection,"Select lastDate from Users_tbl where User_name='"+name+"'",function (ans) {
        if(ans.length==1)
        {
...
router.get('/getNewItems', function(req,res){
    ...
    db.Select(connection, "select Item_id,Item_name,Item_price,Item_Category,Item_stock,datePublish,Item_artist,buyAmount,photourl from Items_tbl where datePublish>='"+dateNow+"'", function (ans) {
        res.send(ans);
    });
...
router.get('/getItemSortByCategory', function (req,res) {
   ...
   // var sortBy = req.param("Item_order");
    db.Select(connection, "select * from Items_tbl where Item_Category='" + cat +"'", function (ans) {
        res.send(ans);
    });
});
